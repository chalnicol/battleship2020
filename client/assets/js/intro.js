...
class Intro extends Phaser.Scene {
    ...
    create () 
    {
        ...
        this.playerOnlineTxt = this.add.text ( 50 , 150, 'Players Online: -', { color:'#6e6e6e', fontFamily:'Oswald', fontSize : 28 }).setOrigin(0);
    ...
    createMenu ( last = false )
    {
        ...
        for ( let i = 0; i < 3; i++ ) {
            ...
            const xp = 960, yp = 610 + (i *( bh+bsp));

            let rct = this.add.container ( xp, 540 + yp ).setSize ( bw, bh).setInteractive ();

            let img = this.add.image ( 0, 0, 'menu' );

            let txt = this.add.text (0, 0, !last ? txtArr [i] : txtArr[ i + 3 ], {color:'#3a3a3a', fontFamily:'Oswald', fontSize:40 }).setOrigin(0.5);
